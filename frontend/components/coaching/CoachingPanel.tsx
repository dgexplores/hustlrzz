...
function RoleFitWorkspace({ form, setForm, result, loading, onSubmit }: { form: any; setForm: (value: any) => void; result: any; loading: boolean; onSubmit: (event: React.FormEvent) => void }) {
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
    <Card><CardHeader><CardTitle className="text-xl">Compare your resume to the role</CardTitle><p className="text-sm leading-6 text-muted-foreground">See the strengths you can prove and the requirements that need stronger evidence.</p></CardHeader><CardContent><form onSubmit={onSubmit} className="space-y-4"><div className="space-y-2"><Label htmlFor="fit-jd">Job description</Label><Textarea id="fit-jd" rows={9} required value={form.job_description} onChange={(event) => setForm({ ...form, job_description: event.target.value })} placeholder="Paste responsibilities, requirements, and preferred skills…" /><p className={`text-xs ${form.job_description.length >= 80 ? "text-muted-foreground" : "text-amber-600 dark:text-amber-400"}`}>{form.job_description.length}/80 minimum for analysis</p></div><div className="space-y-2"><Label htmlFor="fit-resume">Resume or experience summary</Label><Textarea id="fit-resume" rows={9} required value={form.resume_text} onChange={(event) => setForm({ ...form, resume_text: event.target.value })} placeholder="Paste relevant projects, experience, skills, and outcomes…" /><p className={`text-xs ${form.resume_text.length >= 80 ? "text-muted-foreground" : "text-amber-600 dark:text-amber-400"}`}>{form.resume_text.length}/80 minimum for analysis</p></div><Button type="submit" size="lg" disabled={loading || form.job_description.trim().length < 80 || form.resume_text.trim().length < 80} className="w-full">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />Analyzing evidence…</> : <><Sparkles className="h-4 w-4" />Analyze role fit</>}</Button></form></CardContent></Card>
    <Card className="min-h-[700px]"><CardHeader className="flex flex-row flex-wrap items-start justify-between gap-2 space-y-0"><div><CardTitle className="text-xl">Fit analysis</CardTitle><p className="mt-1 text-sm text-muted-foreground">Use this as coaching guidance, not a hiring decision.</p></div>{result && <Button variant="outline" size="sm" onClick={() => downloadJson("hustlrzz-role-fit.json", result)}><Download className="h-4 w-4" />Export</Button>}</CardHeader><CardContent>{result ? <FitResult result={result} /> : <EmptyResult icon={<BriefcaseBusiness className="h-7 w-7" />} title="Your evidence map will appear here" copy="Review matched skills, proof gaps, resume weaknesses, and the next action to take." />}</CardContent></Card>
  </div>;
}
