  ...
  return <main className="mx-auto max-w-7xl space-y-8 px-4 py-8 md:px-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
      <Card className="h-fit"><CardHeader><CardTitle>Analyze a resume</CardTitle><p className="text-sm leading-6 text-muted-foreground">Add a job description for a more focused skills comparison.</p></CardHeader><CardContent><form onSubmit={submit} className="space-y-5">
        <div className="space-y-2"><Label>Resume file</Label><input ref={inputRef} className="sr-only" type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => { const selected = event.target.files?.[0] || null; if (selected && !/\.(pdf|docx)$/i.test(selected.name)) { setError("Upload a PDF or DOCX resume."); event.currentTarget.value = ""; return; } if (selected && selected.size > 5 * 1024 * 1024) { setError("Resume files must be 5 MB or smaller."); event.currentTarget.value = ""; return; } setError(null); setFile(selected); }} />
          <button type="button" onClick={() => inputRef.current?.click()} className="flex min-h-32 w-full flex-col items-center justify-center rounded-xl border border-dashed border-input bg-secondary/20 px-4 text-center surface-transition hover:border-primary/50 hover:bg-primary/5"><Upload className="h-5 w-5 text-primary" /><span className="mt-3 max-w-full break-all text-sm font-semibold">{file ? file.name : "Choose a PDF or DOCX"}</span><span className="mt-1 text-xs text-muted-foreground">Maximum 5 MB</span></button>
        </div>
        ...
        {usageError && <p className="text-center text-xs text-muted-foreground">Usage could not be loaded — the server still enforces your quota.</p>}
    ...
    <section className="pb-8"><div className="mb-4 flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" /><h2 className="text-xl font-semibold">Recent analyses</h2></div><div className="divide-y rounded-xl border bg-card">{history.length ? history.map((item) => (
      <div key={item.analysis_id} className="flex min-w-0 items-stretch">
        <button type="button" onClick={() => openHistory(item.analysis_id)} className="grid min-w-0 flex-1 gap-2 px-4 py-4 text-left surface-transition hover:bg-accent sm:grid-cols-[auto_1fr_auto] sm:items-center"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/10 font-semibold text-primary">{item.resume_score}</span><span className="min-w-0"><span className="block text-sm font-semibold">ATS readiness review</span><span className="mt-1 block break-words text-xs text-muted-foreground">{item.extracted_skills?.slice(0, 4).join(" · ") || "No skills listed"}</span></span><span className="text-xs text-muted-foreground">{item.created_at && !Number.isNaN(new Date(item.created_at).getTime()) ? new Date(item.created_at).toLocaleDateString() : "View"}</span></button>
        <button type="button" aria-label="Delete saved analysis" disabled={deletingId === item.analysis_id} onClick={() => deleteHistory(item.analysis_id)} className="shrink-0 self-center rounded-lg p-3.5 text-muted-foreground surface-transition hover:bg-accent/45 hover:text-destructive">{deletingId === item.analysis_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</button>
      </div>
